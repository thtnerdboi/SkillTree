# Terms of Service for SkillTree
**Last Updated: July 2026**

**1. Acceptance of Terms**
By downloading and using SkillTree, you agree to these Terms of Service.

**2. Subscriptions and Payments (SkillTree Pro)**
- SkillTree offers a "Pro" tier processed via Revenuecat.
- Payments are charged to your designated payment method at the time of purchase.
- You may cancel your subscription at any time, but we do not offer refunds for partial billing periods unless required by law.

**3. Acceptable Use**
SkillTree is designed to help you build habits. You agree not to reverse-engineer the app, exploit the XP system maliciously, or harass other users through the social features.

**4. "As-Is" Service**
SkillTree is provided "as is." While we strive for 100% uptime, we do not guarantee that the app will be free of bugs or server interruptions. 

**5. Account Termination**
We reserve the right to suspend or terminate accounts that violate these terms.

**6. Use of Code**
This app may not be repackaged, altered or changed in any shape or form by a third party without consent.
